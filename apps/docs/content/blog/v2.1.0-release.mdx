...
> **Safety update:** The render-time `silent(() => resource.set(...))` initialization described in
> this historical release is deprecated. React can detect the changed external-store snapshot even
> when subscriber callbacks are suppressed. Use selector `.suspend(arg)` for SSR query data.
