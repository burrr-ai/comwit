...
> **Safety update:** Render-time initialization through `silent()` is deprecated. For data that
> must block server rendering, use a query-backed resource and select `.suspend(arg)` instead.
