...
`PageTransition` lays the shell the leaving page needs (`relative z-0 min-h-dvh overflow-x-clip`); inside your own scroller pass `className="min-h-full"`. Use `clip`, never `overflow-x-hidden`, which would turn the shell into a scroll container. Give every page a background. When an app bar and bottom nav survive tab changes, keep them in an outer boundary with a fixed `routeKey` and put the inner boundary in its own `relative` parent, otherwise the leaving page jumps to the top of the shell:

```tsx
<RouteBoundary routeKey="tabs" className="flex min-h-full flex-col">
  <AppBar behavior="reveal">…</AppBar>
...
`hero()` carries one element between two pages: give it `data-hero-exit-key={id}` where you leave and `data-hero-enter-key={id}` where you arrive. Rules, every preset, persistent layouts, scroll restoration and troubleshooting are documented by [SSGOI](https://ssgoi.dev/llms.txt), the engine behind it; the config shape is identical, only the component names differ.
