...
Standalone helpers exported from `@comwit/state` for working with reactive proxies and inspecting state during development.
...
Returns a detached snapshot of a comwit reactive proxy. Later store mutations do not change it.
Nested reactive objects are snapshotted recursively; properties are non-writable and objects are
non-extensible. Treat the result as read-only and use ordinary actions to change state.
