...
`$history.ignore()` only skips history recording. Every change inside it still notifies subscribers
and rerenders normally; it is the correct API for history control.
