...
No new hook is required. Read and call `$history` from the existing hook created by [`create()`](/state/docs/api/create).
...
This is different from deprecated [`silent()`](/state/docs/api/silent). `silent()` suppresses subscriber
notifications during a synchronous mutation scope. `$history.ignore()` only skips history
recording and is the correct API for history control.
