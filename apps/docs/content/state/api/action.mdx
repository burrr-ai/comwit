...
When a model has [`history: true`](/state/docs/api/history), each action method call is recorded as one history transaction. Multiple state mutations inside the method are undone together via `state.$history.undo()`.
...
- [Decorator](/state/docs/decorators) support (`@OnError`, `@OnSuccess`, `@Debounce`, etc.)
...
Combine them in [`create()`](/state/docs/api/create).
