...
Defines a reactive model. Each provider owns its own state instance; each domain usually has one model.
...
```ts
// With persist fields
import { model, query, persist } from '@comwit/state'
...
- Fields using [`query()`](/state/docs/api/query) become keyed data resources; selectors use `.load()`
  for client fetching and generated domain hooks use `.hydrate()` for resolved server data
- Fields using [`local()`](/state/docs/api/local) restore exact IndexedDB views and share canonical entities
- Fields using [`persist()`](/state/docs/api/persist) are automatically synced to storage
- Fields using [`searchParam()`](/state/docs/api/router) are automatically synced to URL query parameters
- Plain fields (strings, objects, arrays, null) are reactive local state
- Models with `history: true` expose [`$history`](/state/docs/api/history) for undo and redo
- Models initialize lazily — when first accessed through a hook or `state(model)` in an action
...
No new hook is needed. Access `$history` through the existing hook from [`create()`](/state/docs/api/create).
...
Use the hook created by [`create()`](/state/docs/api/create) to read model state:
