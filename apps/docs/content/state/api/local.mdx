...
`local.query()` retains `.load()`, experimental `.suspend()`, `.query()`, `.refetch()`, `.set()`, and
the ordinary query state.
Standalone `local()` exposes `.restore()`, `.set()`, `.remove()`, and the same data/status fields but
has no API driver.

## App Router exact cached fallback
...
### Server-rendered remote data

Standalone `local()` only restores IndexedDB and never starts remote work. For `local.query()` or
`local.infinite()`, await remote data in a Server Component and initialize the resolved entry with
the generated domain hook's `hydrate()` method. IndexedDB reconciliation, persistence, and entity
fanout run after the route adapter commits.

```ts
detail: local.query<ProductDetail | null, { id: string }>({
  source: products,
  initialData: null,
  queryFn: ({ id }) => api.product.detail(id),
})
```

```tsx
function ProductRoute({ id, initialDetail }: Props) {
  useProduct.hydrate({ detail: { arg: { id }, data: initialDetail } })
...
Standalone `local()` is restore-only and is not a `hydrate()` target. Keep `.restore(arg)` for an
exact, non-suspending cached fallback. Experimental `.suspend(arg)` remains available on query-backed
local resources with isomorphic query functions. Do not call a render-body action that writes a
resolved server value through `.set()` or deprecated `silent()`.
...
`query.realtime()` is not supported by the experimental `.suspend()` path.

## Slow initial loading

Set `slowLoadingMs: 500` in `local.query()` or `local.infinite()` to read `isSlowLoading` through
normal query selectors. IndexedDB restoration and a cache miss's remote fetch share one initial
loading interval. An exact durable hit clears the flag immediately; stale background revalidation
keeps it false. The flag is not persisted. Standalone `local()` does not accept the option.
See the [query timing and rendering contract](/state/docs/api/query#slow-initial-loading).
...
adapters degrade to ordinary queries. Server environments skip the IndexedDB lifecycle
without throwing. Selector `.load()` still waits for client commit before starting a request; use
`hydrate()` for resolved server data, or an imperative `.query()` in an appropriate non-render workflow.
