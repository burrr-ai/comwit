...
Inside `useModel()` or a hook created by `create()`, choose one of two selector methods:

- `.load(arg)` is non-suspending. It reports loading state immediately and starts the request after
  commit.
- `.suspend(arg)` is Suspense-native. It starts or reuses a keyed Promise during render and throws
...
Use `.suspend(arg)` when server rendering should wait for the data:

```tsx
function ProductDetail({ slug }: { slug: string }) {
  const detail = useProduct((state) => state.detail.suspend(slug))
  if (!detail.data) return <NotFound />
  return <ProductView product={detail.data} />
}
```

The query Promise is cached by its serialized argument before React suspends, so retries reuse the
same work. A surrounding `<Suspense>` streams its fallback. Without an explicit boundary, the
React 19 streaming server renderer waits at the root and retries when the Promise settles. Errors
propagate to the nearest error boundary.

During browser mounting or hydration, place that boundary below `ComwitProvider`. If the client
root suspends before the Provider commits, React can discard its provider-scoped cache and restart
the query.

## Methods

Inside React selectors, a query exposes `.load(arg?)` and `.suspend(arg?)`. Once accessed via
...
| `.suspend(arg?)`       | Selector | Start during render and suspend until the keyed query settles.                    |
...
| `suspense`        | **Deprecated.** Use selector `.suspend(arg)` instead.                                            |
...
`suspend()` is the explicit blocking read. It prepares the key-scoped query Promise during render,
...
On the server-rendered pass of a Next.js Client Component, `suspend()` starts the query before its
...
renders. Legacy `renderToString()` and framework-specific cache rules are outside this contract.
...
option, uses `.load()` for the non-suspending path, and selects `.suspend()` at the call site when
React should wait.
