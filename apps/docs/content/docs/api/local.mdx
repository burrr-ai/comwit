...
The resolved server branch initializes the same exact view after the Client Component commits.
Include the argument because the server branch may resolve before the fallback has activated the
resource.
...
function ProductInit({ id, value }: { id: string; value: ProductDetail }) {
  ...
  useHydrate(() => actions.initDetail(id, value), [id, value, actions])
  return <ProductView product={value} />
}
```

`useHydrate()` keeps render free of external-store mutations. After commit, `set()` marks the
resource successful, writes the exact detail view and canonical entity to IndexedDB, and fans
shared fields out to loaded product lists.
...
server detail fetch starts
  ...
  │    └─ miss → skeleton
  └─ server resolves
       → commit: useHydrate(detail.set(serverValue, { arg: { id } }))
