...
`local.query()` retains `.load()`, `.suspend()`, `.query()`, `.refetch()`, `.set()`, and the ordinary
query state.
...
Standalone `local()` only restores IndexedDB and never starts remote work. Use `local.query()` or
`local.infinite()` with `.suspend(arg)` when remote data must block server rendering. The remote
result is staged in the key cache during render; IndexedDB reconciliation, persistence, and entity
fanout run only after the rendered tree commits.
...
function ProductDetail({ id }: { id: string }) {
  const detail = useProduct((state) => state.detail.suspend({ id }))
  if (!detail.data) return <NotFound />
  return <ProductView product={detail.data} />
}
```

Standalone `local()` does not expose `.suspend()`. Keep `.restore(arg)` for an exact,
non-suspending cached fallback. Do not call a render-body action that writes a resolved server value
through `.set()` or deprecated `silent()`; that mutates an external-store snapshot React may
already be checking.
