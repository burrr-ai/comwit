...
Do not call a mutating action directly during render. For data that must resolve during server
rendering, model it as a [`query()`](/docs/api/query) and call `.suspend(arg)` from the consuming
selector. Actions remain the right place for user events, imperative preloads, and coordinated
workflows.
