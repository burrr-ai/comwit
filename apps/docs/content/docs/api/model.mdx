...
- Fields using [`query()`](/docs/api/query) become keyed data resources; selectors use `.load()`
  for client fetching and generated domain hooks use `.hydrate()` for resolved server data
- Fields using [`local()`](/docs/api/local) restore exact IndexedDB views and share canonical entities
- Fields using [`persist()`](/docs/api/persist) are automatically synced to storage
- Fields using [`searchParam()` (beta)](/docs/api/router) are automatically synced to URL query parameters
