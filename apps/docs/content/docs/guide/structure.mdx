...
export type PostState = {
  ...
  current: Query<Post | null, string> // use suspend(slug) when SSR must wait
}
...
For view-owned requests, call `s.posts.load(arg)` for non-suspending client loading or
`s.posts.suspend(arg)` when server rendering must wait for the query. Keep a `loadPosts()` action
when fetching is part of a command, multi-query sequence, preload, or user event.
