...
> Migrating from the old `comwit` package? See the [v2.0.0 release notes](/blog/v2.0.0-release). The API is identical — only the package name changed.
...
```tsx
import { OnError, Log, ComwitProvider } from '@comwit/state'
