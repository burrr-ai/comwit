...
Most state libraries were designed for humans reading docs. mucha was designed for LLMs writing code. That changes everything about how a library should be structured.

---

## Optimized for vibe coding

You don't learn a state library anymore. Your LLM does. mucha is architected so that LLMs spend fewer tokens understanding context and generating correct code. Less token overhead means faster results, fewer hallucinations, and less back-and-forth. The library is the prompt.

## Next.js native

Not a generic React library bolted onto Next.js after the fact. mucha was built from day one for App Router, server components, and SSR. State initializes on the server without hydration mismatches, without workarounds, without ceremony.
...
mucha is a set of hard opinions about how state should be organized — opinions that make code easier to write, easier to read, and dramatically easier for machines to generate correctly.
