...
Not a generic React library bolted onto Next.js after the fact. comwit was built for App Router,
server components, and SSR. A Server Component can pass a resolved query seed to a small client
adapter, which initializes the domain cache with `useDomain.hydrate()` before the UI reads it;
ordinary `load()` keeps the familiar client loading-state flow.
