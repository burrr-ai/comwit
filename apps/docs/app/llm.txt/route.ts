...
The pattern above can be automated with \`createInterceptor\`. See Decorators below.
...
\`createInterceptor(({ state, context }) => MethodDecorator)\` — reusable decorator with \`state\`/\`context\` access, like action factories.
