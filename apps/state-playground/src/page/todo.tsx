...
import type { Filter } from '@/state/todo/types'
...
function FilterBar({ forceFail }: { forceFail: boolean }) {
  ...
  return (
    ...
    >
      ...
      <select
        value={filter.status}
        onChange={(e) => actions.setFilter({ status: e.target.value as Filter['status'] })}
        style={{ padding: '4px 8px', border: '1px solid #999', background: '#fff', color: '#000' }}
      >
      ...
      <select
        ...
        onChange={(e) => actions.setFilter({ priority: e.target.value as Filter['priority'] })}
        style={{ padding: '4px 8px', border: '1px solid #999', background: '#fff', color: '#000' }}
      >
...
function TodoItem({ todo, forceFail }: { todo: Todo; forceFail: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 12, color: '#666' }}>{todo.priority}</span>
      <select
        value={todo.status}
        onChange={async (e) => {
          try {
...
function TodoList({ forceFail }: { forceFail: boolean }) {
  ...
  return (
    <div>
      {filtered.map((todo) => (
        <TodoItem key={todo.id} todo={todo} forceFail={forceFail} />
      ))}
    </div>
  )
}
...
function ResourceSection() {
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 8, borderTop: '1px solid #f3f4f6', paddingTop: 8 }}>
        ...
        >
          <p style={{ margin: 0, fontSize: 12, color: '#6b7280' }}>
            Collaborators{' '}
            {collaborators.isLoading ? 'loading...' : `(${collaborators.data.totalCount ?? 0})`}
          </p>
          <button
        ...
        {collaborators.isError && (
          <button
            onClick={() => {
              void actions.loadFirstCollaborators()
            }}
            style={{
              marginTop: 6,
      ...
      {me.isError && (
        <p style={{ margin: '6px 0 0', color: 'crimson', fontSize: 12 }}>
          user error: {String(me.error)}
        </p>
      )}
    </section>
  )
