...
1. Implement scope-aware transaction context in `packages/comwit/src/core/provider.tsx` and `packages/comwit/src/core/index.ts` so action execution can run inside a transaction.
2. Track accessed models inside `packages/comwit/src/core/action.ts` (or action execution boundary) and expose scope to interceptors.
3. Update `packages/comwit/src/interceptors/transaction.ts` to:
...
4. Add model versioning helpers in `packages/comwit/src/core/proxy.ts` and export from `packages/comwit/src/utils/index.ts` for transaction checks.
5. Define a conflict path in `packages/comwit/src/interceptors/transaction.ts` (e.g., `TransactionConflict` error) to avoid blind overwrite when concurrent writes happened.
6. Add `undo`/`redo` notes and minimal API in `packages/comwit/src/interceptors/transaction.ts` (or a follow-up file) using change logs/inverse patches.
7. Clean docs in `packages/comwit/README.md` by keeping `Transaction` as an experimental/placeholder API only (no usage example).
