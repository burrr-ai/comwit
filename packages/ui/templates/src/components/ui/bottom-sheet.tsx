 ...
 * `--sheet-drag-progress` 만큼 옅어지고, 닫힘 애니메이션은 끌린 자리에서 이어진다.
 *
 *   <BottomSheet>
  ...
  return (
    <BottomSheetPrimitive.Overlay
      ...
      className={cn(
        ...
        // 끌수록 옅어진다(프리미티브가 쓰는 진행도). 끄는 동안은 전환 없이 바로.
        'opacity-[calc(1-var(--sheet-drag-progress,0))] transition-opacity duration-slow data-[dragging]:transition-none',
      ...
      <GlassSurface variant="morphing" shape="panel" dense>
        <BottomSheetPrimitive.Content
          ...
          className={cn(
            ...
            // 손가락을 따라오는 translate — 끄는 동안은 전환 없이, 놓으면 제자리로 돌아온다.
            'translate-y-(--sheet-drag) transition-transform ease-[cubic-bezier(0.22,1,0.36,1)] data-[dragging]:transition-none',
            // 열림·닫힘. 드래그로 닫히면 끌린 자리에서 이어서 내려간다.
            'data-[state=open]:animate-in data-[state=open]:slide-in-from-bottom data-[state=open]:duration-slower data-[state=closed]:animate-out data-[state=closed]:slide-out-to-bottom data-[state=closed]:duration-slow',
            className
          )}
          {...props}
        >
